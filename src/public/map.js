...
function bottomController() {
  ...
  `;
        ...
        link.onclick = function (e) {
            ...
            if (
                (visibility === "visible" || visibility === undefined) &&
                checkbox.checked
            ) {
    ...
    map.on("style.load", () => {
        map.on("click", (e) => {
            ...
            if (
                ...
                features[0].properties.name !== undefined
            ) {
                if (
                    features[0].layer.id == "unclustered-point" ||
                    features[0].layer.id == "unclustered-point-label"
                ) {
                    return;
                }
    ...
    map.on("load", () => {
        ...
        (async () => {
            ...
            map.on("click", "unclustered-point", (e) => {
                ...
                document.getElementById(
                    "place-info-pane"
                ).innerHTML = `${placeInfoPaneHeader}<br><strong>${e.features[0].properties.name}</strong><br>${address}<br><br><img class="img-fluid" src=${imgUrl} alt=""><br><br>${reportButton}`;
        ...
        .then((data) => {
            if (data.features && data.features.length > 0) {
                ...
                const placeInfoPaneHeader =
                    '<h5 class="alert-heading"><i class="bi bi-check2-circle"></i> Thông tin địa điểm</h5>';
                const reportButton =
                    '<button type="button" class="btn btn-outline-danger" href="/olala"><i class="bi bi-exclamation-octagon-fill"></i> BÁO CÁO VI PHẠM</button>';
