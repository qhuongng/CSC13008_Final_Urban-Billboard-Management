...
function setupMap(center) {
    ...
    map.on("load", async () => {
        ...
        if (authUser.role[0] != "-1") {
            await fetch(`https://officer-mapapp.vercel.app/api/ward/getDetail-ward/${authUser.role[1]}`)
                .then((response) => response.json())
                .then((data) => {
                    for (let i = 0; i < data.data.length; i++) {
                ...
                if (props.pointReport === true) {
                    ...
                    for (let i = 0; i < pointReports.length; i++) {
                        ...
                        if (info.reportPicture.length === 2) {
                            imgDivs += `<img class="img-fluid" src="https://officer-mapapp.vercel.app/api/reportImg/getImgReport/${info.reportPicture[0]}"" alt=""><br>
                                        <img class="img-fluid" src="https://officer-mapapp.vercel.app/api/reportImg/getImgReport/${info.reportPicture[1]}"" alt=""><br><br>`
                        }
                        else if (info.reportPicture.length === 1) {
                            imgDivs += `<img class="img-fluid" src="https://officer-mapapp.vercel.app/api/reportImg/getImgReport/${info.reportPicture[0]}"" alt=""><br><br>`
                        }
